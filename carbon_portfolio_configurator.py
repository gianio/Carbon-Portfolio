...
# Allocation Function
# ==================================
# (Allocation function remains unchanged)
...
with st.sidebar:
    ...
    if df_upload:
        ...
        def load_and_prepare_data(uploaded_file):
            # (load_and_prepare_data function remains unchanged)
            try: data = pd.read_csv(uploaded_file); data.columns = data.columns.str.lower().str.strip().str.replace(' ', '_')
            ...
            for col in data.columns:
                for prefix in numeric_prefixes_std:
                    ...
                    if col.startswith(prefix) and year_part.isdigit(): cols_to_convert_numeric.append(col); year_data_cols_found.append(col); available_years.add(int(year_part)); break
            if not available_years:
                 has_price_prefix = any(c.startswith('price_') for c in data.columns); has_vol_prefix = any(c.startswith('available_volume_') for c in data.columns)
            ...
            if 'project_type' in data.columns:
                ...
                valid_types = ['reduction', 'technical removal', 'natural removal']; invalid_types_df = data[~data['project_type'].isin(valid_types)]
            ...
            else: return None, "Critical error: 'project_type' column missing despite initial check passing.", available_years, [], []
            cols_to_keep = core_cols_std[:];
            for col in optional_cols_std:
                if col in data.columns: cols_to_keep.append(col)
            cols_to_keep.extend(year_data_cols_found); data = data[list(set(cols_to_keep))]
            final_rename_map = {'project_name': 'project name', 'project_type': 'project type', 'priority': 'priority', 'description': 'Description', 'project_link': 'Project Link'}
            for yr_col in year_data_cols_found: final_rename_map[yr_col] = yr_col.replace('_', ' ')
            rename_map_for_df = {k: v for k, v in final_rename_map.items() if k in data.columns}; data.rename(columns=rename_map_for_df, inplace=True)
            project_names_list = []
    ...
    if st.session_state.get('data_loaded_successfully', False):
        ...
        if not available_years_in_data:
            ...
            if not st.session_state.selected_years:
                st.sidebar.error(f"No data available for selected period ({start_year_selected}-{end_year_selected}). Adjust 'Years to Plan' slider or check CSV data.")
                ...
                # --- Annual Target Inputs (Foldable Expander) --- START OF CORRECTED BLOCK ---
                st.markdown("### Annual Target Settings")
                # Use a master target input as a default for individual years
                master_target_value = st.session_state.get('master_target')

                if constraint_type == 'Volume':
                    # Try to provide a sensible default, potentially based on previous value if type matches
                    default_val_vol = 1000 # A reasonable starting point
                    # CORRECTED SYNTAX: Put try-except on separate lines
                    if master_target_value is not None and isinstance(master_target_value, (int, float)):
                        try:
                             default_val_vol = int(float(master_target_value))
                        except (ValueError, TypeError):
                             pass # Keep the initial default_val_vol if conversion fails
                    default_target = st.number_input(
                        "Default Annual Target Volume (t):",
                        min_value=0, step=100,
                        value=default_val_vol,
                        key='master_volume_sidebar', # Key specific to volume
                        help="Set a default target volume per year. You can override specific years below."
                        )
                else: # Budget constraint
                    default_val_bud = 100000.0 # Reasonable starting budget
                    # CORRECTED SYNTAX: Put try-except on separate lines
                    if master_target_value is not None and isinstance(master_target_value, (int, float)):
                         try:
                             default_val_bud = float(master_target_value)
                         except (ValueError, TypeError):
                             pass # Keep the initial default_val_bud if conversion fails
                    default_target = st.number_input(
                        "Default Annual Target Budget (€):",
                        min_value=0.0, step=1000.0,
                        value=default_val_bud, format="%.2f", # Format for currency
                        key='master_budget_sidebar', # Key specific to budget
                        help="Set a default target budget per year. You can override specific years below."
                        )
                st.session_state.master_target = default_target # Store the current default

                # Expander for customizing targets per year
                with st.expander("Customize Annual Targets (+/-)", expanded=False):
                    current_annual_targets = st.session_state.get('annual_targets', {})
                    updated_targets_from_inputs = {} # Store values from the input fields in this run

                    if not st.session_state.selected_years:
                        st.caption("Select years using the 'Years to Plan' slider above first.")
                    else:
                         # Create input fields for each selected year
                         for year in st.session_state.selected_years:
                             # Use the existing target for the year, or the current default master target
                             year_target_value = current_annual_targets.get(year, default_target)
                             # Use constraint type in the key to handle switching between Volume/Budget
                             input_key = f"target_{year}_{constraint_type}"
                             label = f"Target {year} [t]" if constraint_type == 'Volume' else f"Target {year} [€]"

                             if constraint_type == 'Volume':
                                 try: input_val = int(year_target_value) # Ensure correct type
                                 except (ValueError, TypeError): input_val = int(default_target) # Fallback
                                 updated_targets_from_inputs[year] = st.number_input(
                                     label, min_value=0, step=100, value=input_val, key=input_key
                                     )
                             else: # Budget
                                 try: input_val = float(year_target_value) # Ensure correct type
                                 except (ValueError, TypeError): input_val = float(default_target) # Fallback
                                 updated_targets_from_inputs[year] = st.number_input(
                                     label, min_value=0.0, step=1000.0, value=input_val, format="%.2f", key=input_key
                                     )
                    # Update the session state with the latest values from the input fields
                    st.session_state.annual_targets = updated_targets_from_inputs
                # --- End Annual Target Inputs --- END OF CORRECTED BLOCK ---
                ...
                else: st.sidebar.info("Transition inactive: No 'Reduction' projects found.")
                ...
                if not project_names_list: st.sidebar.warning("No projects available.")
                    ...
                    if 'priority' in data_for_ui.columns:
                        ...
                        if boost_options:
                            current_favorite = st.session_state.get('favorite_projects_selection', []); valid_default_favorite = [f for f in current_favorite if f in boost_options][:1]
...
if st.session_state.get('data_loaded_successfully', False):
    if not st.session_state.get('selected_projects'):
        ...
        if keys_present:
            try:
                fav_proj = st.session_state.favorite_projects_selection[0] if st.session_state.favorite_projects_selection else None; constraint = st.session_state.constraint_type
                min_chunk = st.session_state.min_alloc_chunk; annual_targets_to_use = st.session_state.annual_targets
                ...
                details_df = pd.DataFrame(details_list); pivot_display = pd.DataFrame()
                st.markdown("## Portfolio Summary"); col_l, col_r = st.columns([2, 1.2], gap="large")
                with col_l:
                    ...
                    if not summary.empty:
                         ...
                         st.markdown(f"""<div class="metric-box"><b>Total Portfolio Cost</b> - </div>""", unsafe_allow_html=True); st.markdown(f"""<div class="metric-box"><b>Total Portfolio Volume</b> - </div>""", unsafe_allow_html=True)
                         st.markdown(f"""<div class="metric-box"><b>Overall Average Price</b> - </div>""", unsafe_allow_html=True); st.warning("Could not calculate overall portfolio metrics.")
                with col_r:
                    st.markdown("#### Volume by Project Type")
                    if not details_df.empty:
                        pie_data = details_df.groupby('type')['volume'].sum().reset_index(); pie_data = pie_data[pie_data['volume'] > 1e-6]
                        ...
                        elif not details_df.empty and pie_data.empty: st.caption("No significant volume allocated for pie chart.")
                        else: st.caption("Could not generate pie chart data.")
                ...
                elif details_df.empty: st.warning("No detailed project allocations for composition plot.")
                else:
                    st.markdown("### Portfolio Composition & Price Over Time")
                    summary_plot_data = details_df.groupby(['year', 'type']).agg(volume=('volume', 'sum'), cost=('cost', 'sum')).reset_index(); price_summary_data = pd.DataFrame()
                    if not summary.empty: price_summary_data = summary[['Year', 'Avg. Price']].rename(columns={'Year':'year', 'Avg. Price':'avg_price'})
                    fig_composition = make_subplots(specs=[[{"secondary_y": True}]]); y_metric = 'volume' if constraint == 'Volume' else 'cost'; y_label = 'Allocated Volume (t)' if constraint == 'Volume' else 'Allocated Cost (€)'
                    y_format = '{:,.0f}' if constraint == 'Volume' else '€{:,.2f}'; y_hover_label = 'Volume' if constraint == 'Volume' else 'Cost'; type_order = ['reduction', 'natural removal', 'technical removal']; types_in_results = details_df['type'].unique()
                    for t_name in type_order:
                        if t_name in types_in_results:
                            df_type = summary_plot_data[summary_plot_data['type'] == t_name]
                            if not df_type.empty and y_metric in df_type.columns and df_type[y_metric].sum() > 1e-6: fig_composition.add_trace(go.Bar(x=df_type['year'], y=df_type[y_metric], name=t_name.replace('_', ' ').capitalize(), marker_color=type_color_map.get(t_name, default_color), hovertemplate=f'Year: %{{x}}<br>Type: {t_name.replace("_", " ").capitalize()}<br>{y_hover_label}: %{{y:{y_format}}}<extra></extra>'), secondary_y=False)
                ...
                if not details_df.empty or not summary.empty:
                    try:
                        pivot_final = pd.DataFrame(); years_present_in_results = st.session_state.selected_years
                        if not details_df.empty:
                            ...
                            if not pivot_intermediate.empty:
                                pivot_final = pivot_intermediate.swaplevel(0, 1, axis=1); metric_order = ['volume', 'cost', 'price']
                                years_present_in_pivot = sorted(pivot_final.columns.get_level_values(0).unique()); years_present_in_results = years_present_in_pivot
                                final_multi_index = pd.MultiIndex.from_product([years_present_in_results, metric_order], names=['year', 'metric'])
                                pivot_final = pivot_final.reindex(columns=final_multi_index); pivot_final = pivot_final.sort_index(axis=1, level=[0, 1])
                        ...
                        total_row_index = pd.MultiIndex.from_tuples([('Total Portfolio', 'All Types')], names=['Project Name', 'Type']); total_row_df = pd.DataFrame(total_data, index=total_row_index)
                        if pivot_final.empty: pivot_display = total_row_df
                        else: pivot_display = pd.concat([pivot_final, total_row_df])
                        pivot_display = pivot_display.fillna(0); formatter = {}
                ...
                if not pivot_display.empty:
                    csv_df = pivot_display.copy(); csv_df.columns = [f"{int(col[0])}_{col[1]}" for col in csv_df.columns.values]; csv_df = csv_df.reset_index()
                    csv_string = csv_df.to_csv(index=False).encode('utf-8'); st.markdown("---")
                    # Download button - now on the left by default
            ...
            except Exception as e: st.error(f"Unexpected error: {e}"); st.error(f"Traceback: {traceback.format_exc()}")
        else: st.error("Missing required settings. Reload data & configure settings.")
