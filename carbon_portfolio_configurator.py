...
if df_upload:
    ...
    with col2:
        ...
        if selected_projects:
            ...
            # Final Year Removal %
            final_year_summary = summary_df[summary_df['Year'] == end_year]
            final_removal_volume = 0
            for r_type in global_removal_types:
                final_removal_volume += final_year_summary[f'Volume {r_type.capitalize()}'].sum()
            final_total_volume = final_year_summary['Total Volume (tonnes)'].sum()

            if final_total_volume > 0:
                final_removal_percentage = (final_removal_volume / final_total_volume) * 100
                target_percentage_display = removal_target * 100
                progress_color = primary_green if final_removal_percentage >= target_percentage_display else '#FFDDC1' # Light orange if below target

                st.subheader(f"Portfolio Performance in {end_year}")
                col_perf1, col_perf2 = st.columns(2)
                with col_perf1:
                    st.metric(label=f"Total Volume in {end_year}", value=f"{final_total_volume:.0f} tonnes")
                with col_perf2:
                    st.metric(label=f"Removal Volume in {end_year}", value=f"{final_removal_volume:.0f} tonnes")

                st.markdown(f"**Removal Target by {end_year}: {target_percentage_display:.0f}%**")
                st.markdown(f"**Actual Removal Percentage in {end_year}: <span style='color:{text_green}; font-size:24px;'>{final_removal_percentage:.1f}%</span>**", unsafe_allow_html=True)

                progress_value = min(100, final_removal_percentage / removal_target if removal_target > 0 else 0)
                st.progress(progress_value / 100)

            elif selected_years:
                st.info(f"No projects allocated for {end_year}, cannot calculate final removal percentage.")

        else:
            st.info("Select projects to see the portfolio allocation.")
