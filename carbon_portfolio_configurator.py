...
with st.sidebar:
    ...
    if df_upload:
        ...
        def load_and_prepare_data(uploaded_file):
            ...
            # --- Using explicit loop for missing_essential check ---
            missing_essential = [] 
            ...
            expected_std_cols_to_keep = list(set(core_cols_std + margin_cols_std + optional_cols_std + year_data_cols_found_list))
            # Filter the DataFrame to only keep columns that are currently in data.columns AND are in our expected list
            # This removes any truly extraneous columns not declared anywhere.
            data = data[[col for col in data.columns if col in expected_std_cols_to_keep]]
            ...
            # Apply renaming only for columns that currently exist in `data` (which has standardized names)
    ...
    if st.session_state.get('data_loaded_successfully', False):
        ...
        if not years_ui: st.sidebar.warning("No usable year data. Settings disabled.")
            ...
            # Ensure data_ui is the DataFrame with display names for checking price/volume columns
...
elif st.session_state.get('data_loaded_successfully', False): # For Project Offerings
...
# Main calculation/display logic (this block should also be guarded by data_loaded_successfully)
if st.session_state.get('data_loaded_successfully', False):
                    ...
                    if not df_for_pivot_main.empty and 'year' in df_for_pivot_main.columns:
                        try:
                            ...
                            if not pivot_display_run.empty:
                                try:
                                    if isinstance(pivot_display_run.columns, pd.MultiIndex) and len(pivot_display_run.columns.names) == 2 and pivot_display_run.columns.nlevels == 2: # Check number of levels too
                                    ...
                                    if has_margin_metric:
                                        ...
                                        # st.warning("Detailed Table: 'margin' metric data not found or column structure issue. 'Total Margin' set to 0.")
