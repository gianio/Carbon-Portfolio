...
with st.sidebar:
    ...
    if df_upload:
        ...
        def load_and_prepare_data(uploaded_file):
            ...
            standardized_columns_found = [] # Initialize
            try:
                print("--- Attempting to read CSV with utf-8-sig ---")
                # Keep a reference to original columns before standardization
                initial_df = pd.read_csv(uploaded_file, sep=';', encoding='utf-8-sig')
                print(f"DEBUG: Initial columns before any standardization: {initial_df.columns.tolist()}")
                print(f"DEBUG: First 5 rows of initial_df:\n{initial_df.head()}")

                data = initial_df.copy() # Work with a copy
                data.columns = data.columns.str.lower().str.strip().str.replace(r'\s+', '_', regex=True)
                standardized_columns_found = data.columns.tolist()
                print(f"DEBUG: Columns after first standardization attempt: {standardized_columns_found}")

            except Exception as read_error:
                print(f"--- ERROR during first read attempt: {read_error} ---")
                print("--- Attempting to read CSV with default encoding (fallback) ---")
                try:
                    if hasattr(uploaded_file, 'seek'): uploaded_file.seek(0)
                    initial_df_fallback = pd.read_csv(uploaded_file, sep=';')
                    print(f"DEBUG: Initial columns (fallback read) before any standardization: {initial_df_fallback.columns.tolist()}")
                    print(f"DEBUG: First 5 rows of initial_df_fallback:\n{initial_df_fallback.head()}")

                    data = initial_df_fallback.copy() # Work with a copy
                    data.columns = data.columns.str.lower().str.strip().str.replace(r'\s+', '_', regex=True)
                    standardized_columns_found = data.columns.tolist()
                    print(f"DEBUG: Columns after fallback standardization attempt: {standardized_columns_found}")

                except Exception as read_error_fallback:
                    err_msg_debug = f"Error reading/processing CSV (delimiter=';'): {read_error} / {read_error_fallback}"
                    print(f"DEBUG: {err_msg_debug}")
                    return None, err_msg_debug, [], [], []

            if data is None: # Should not happen if previous except returned, but as a safeguard
                 return None, "Failed to load data after multiple attempts.", [], [], []
            ...
            # --- DETAILED DEBUGGING FOR MISSING COLUMNS ---
            print(f"DEBUG: Standardized columns found (for check): {standardized_columns_found}")
            print(f"DEBUG: Core columns expected (hardcoded): {core_cols_std}")
            # Forcing a re-check and detailed print for each expected column
            temp_missing = []
            for core_col_item in core_cols_std:
                is_present = core_col_item in standardized_columns_found
                print(f"DEBUG: Checking for '{core_col_item}': Present = {is_present}")
                if not is_present:
                    temp_missing.append(core_col_item)
            # --- END DETAILED DEBUGGING ---

            missing_essential = [col for col in core_cols_std if col not in standardized_columns_found]
            print(f"DEBUG: Calculated 'missing_essential' list: {missing_essential}")


            if missing_essential:
                found_cols_str = ", ".join(standardized_columns_found)
                error_message = f"CSV missing essential: {', '.join(missing_essential)}. Expected after std. FOUND: [{found_cols_str}]. Check CSV headers & delimiter (';')."
                print(f"DEBUG: Raising error with message: {error_message}") # Print the exact message being returned
                # Also print a more raw representation of found columns to catch subtle issues
                print(f"DEBUG: Raw standardized_columns_found that led to error: {repr(standardized_columns_found)}")
                print(f"DEBUG: Raw core_cols_std for comparison: {repr(core_cols_std)}")
            ...
            if 'treshold_price' in data.columns and 'threshold_price' not in data.columns:
                ...
                standardized_columns_found = data.columns.tolist() # Update if changed
            ...
            data = data[[col for col in all_defined_cols if col in data.columns]] # Select only relevant columns
            ...
            if keys_ok:
                try:
                    ...
                    if not annual_targets_run and st.session_state.selected_years:
                        annual_targets_run = {yr: 0 for yr in st.session_state.selected_years} # Ensure it's a dict
                    ...
                    if df_for_plots_tables.empty : st.warning("No detailed project allocations for plots/tables.")
                        ...
                        if not df_for_pivot.empty and 'year' in df_for_pivot.columns: # Check if 'year' exists
                            try:
                                ...
                                if not summary_df_run.empty and 'Year' in summary_df_run.columns and yrs_pivot_run_list: # Check yrs_pivot_run_list is not empty
                                    total_data_dict_run = {}
                                ...
                                if not pivot_display_run.empty:
                                    ...
                                    else: pivot_display_run['Total Margin'] = 0.0 # Ensure column exists even if no margin data
